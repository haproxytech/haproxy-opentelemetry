...
sh_probe_vars ()
{
	_var_cfg="${SH_TMPDIR}/_probe-vars.cfg"
	_var_out="${SH_TMPDIR}/_probe-vars.log"

	{
		echo "[parser]"
		echo "    otel-instrumentation parser-instr"
		echo "        config $(realpath -L "${SH_CONFDIR}/otel.yml")"
		echo "        scopes parser-scope"
		echo
		echo "    otel-scope parser-scope"
		echo "        span \"parser-span\" root"
		echo "            inject probectx use-vars"
		echo "        otel-event on-frontend-http-request"
	} >"${_var_cfg}"

	if OTEL_PARSER_CFG="${_var_cfg}" "${SH_ARG_HAPROXY}" -c -f "${SH_CONFDIR}/haproxy.cfg" >"${_var_out}" 2>&1; then
		SH_HAS_VARS="true"
	elif ! grep -q "invalid context storage type" "${_var_out}"; then
		echo "${SH_NAME}: cannot tell whether the executable takes 'use-vars'" >&2
		sed -e 's/^/       /' "${_var_out}" >&2
...
sh_probe_yaml ()
{
	_var_out="${SH_TMPDIR}/_probe-yaml.log"

	if ! "${SH_ARG_HAPROXY}" -vv >"${_var_out}" 2>&1; then
		echo "${SH_NAME}: cannot read the build options of the executable" >&2
		sed -e 's/^/       /' "${_var_out}" >&2

		return 1
	fi

	if ! grep -q "OpenTelemetry support" "${_var_out}"; then
		echo "${SH_NAME}: the executable carries no OTel filter" >&2

		return 1
	fi

	# The stand-in of dummy/ reports this C++ version and reads no YAML.
	grep -q "C++ version none" "${_var_out}" || SH_HAS_YAML="true"
...
#
# The 'use-vars' context storage exists only in an executable built with
# OTEL_USE_VARS=1, and the OTel configuration file is read only by one linked
# against the wrapper library, so the cases needing either are held against the
# build at hand.
#
sh_probe_vars || exit ${SH_EX_SOFTWARE}
sh_probe_yaml || exit ${SH_EX_SOFTWARE}
...
while IFS='|' read -r _var_name _var_want _var_match _var_mode _var_kw _var_rule _var_warn _var_nowarn _var_noalert _var_need; do
	if test -n "${SH_ARG_FILTER}"; then
	...
	# A case may ask for a build the executable at hand is not.
	_var_skip=
	if test "${_var_need}" = "vars" -a "${SH_HAS_VARS}" != "true"; then
		_var_skip="the executable does not take 'use-vars'"
	elif test "${_var_need}" = "yaml" -a "${SH_HAS_YAML}" != "true"; then
		_var_skip="the executable does not read the OTel configuration file"
	fi

	# The coverage reports count a skipped case, only its run is left out.
	if test -n "${_var_skip}"; then
		SH_SKIP=$((SH_SKIP + 1))

		echo "SKIP [${_var_name}] ${_var_skip}" >>"${SH_LOG}"
		echo "SKIP [${_var_name}]"
...
echo
echo "PASS ${SH_PASS}, FAIL ${SH_FAIL}, SKIP ${SH_SKIP}, log ${SH_LOG}"
