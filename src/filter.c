...
static int flt_otel_ops_check(struct proxy *p, struct flt_conf *fconf)
{
	...
	list_for_each_entry(conf_scope, &(conf->scopes), list) {
		if (conf_scope->flag_used) {
			 ...
			 * too late for, and those HAProxy skips when the
			 * frontend is the backend.  The body analyser is a
			 * frontend-phase one unless the backend asks for the
			 * body itself, which schedules it after backend
			 * selection.
			 */
			flag_fe_phase = (conf_scope->event == FLT_OTEL_EVENT__STREAM_START) ||
			                ((flt_otel_event_data[conf_scope->event].an_bit & FLT_OTEL_AN_REQ_FE_ONLY) != 0) ||
			                (((flt_otel_event_data[conf_scope->event].an_bit & AN_REQ_HTTP_BODY) != 0) && !FLT_OTEL_PROXY_WANTS_BODY(p));
			 ...
			 * start callback for it.  The stream stop callback misses
			 * it at the other end, HAProxy releasing such a filter as
			 * soon as both channels have ended their analysis, which
			 * it reaches on every path.  Warn rather than fail: the
			 * same OTel configuration may be shared with other proxies.
			...
			else if (!(p->cap & PR_CAP_FE) && (conf_scope->event == FLT_OTEL_EVENT__STREAM_STOP)) {
				FLT_OTEL_WARNING("'%s' : " FLT_OTEL_PARSE_SECTION_SCOPE_ID " '%s' uses event '%s' that runs after a filter "
				                 "of backend proxy '%s' is released, so it never fires there",
				                 conf->id, conf_scope->id, flt_otel_event_data[conf_scope->event].name, p->id);
			}
			else {
 ...
 *   event depending on the <chn> channel direction.  On the channel that ends
 *   its analysis last it also fires the server-unavailable event, when the
 *   response channel never started its analysis, that is when no server was
 *   reached for the stream.
 *
 * RETURN VALUE
 *   Returns a negative value if an error occurs, 0 if it needs to wait,
 *   any other value otherwise.
...
static int flt_otel_ops_channel_end_analyze(struct stream *s, struct filter *f, struct channel *chn)
{
	...
	/*
	 * A response channel that never started its analysis means no server
	 * was reached: HAProxy answered in its place, or the stream ended
	 * before a connection.  The event stands in for the response events
	 * then.  The analyser bits recorded on the runtime context cannot tell
	 * this apart, as a raw TCP stream runs no response analyser at all
	 * unless the proxy has rules.
	 *
	 * The other channel still carries CF_FLT_ANALYZE while its own end of
	 * analysis is pending, so the test below runs on the channel that ends
	 * last, the only point where the flag is final: a connection made
	 * after the request channel ended still starts the response analysis,
	 * which a raw TCP stream does whenever the client stops writing before
	 * the server answers.  The event keeps the request channel, the one it
	 * is bound to, whichever channel carries the call.
	 */
	chn_other = (chn->flags & CF_ISRESP) ? &(s->req) : &(s->res);
	if (!FLT_OTEL_RT_CTX(f->ctx)->flag_res_started && !(chn_other->flags & CF_FLT_ANALYZE)) {
		rc = flt_otel_event_run(s, f, &(s->req), FLT_OTEL_EVENT_REQ_SERVER_UNAVAILABLE, &err);
