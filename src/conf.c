...
FLT_OTEL_CONF_FUNC_FREE(span, id,
	...
	FLT_OTEL_LIST_DESTROY(exception, &((*ptr)->exceptions));
)


/***
 * NAME
 *   flt_otel_conf_exception_init - conf_exception structure allocation
 *
 * SYNOPSIS
 *   struct flt_otel_conf_exception *flt_otel_conf_exception_init(const char *id, int line, struct list *head, char **err)
 *
 * ARGUMENTS
 *   id   - identifier string to duplicate
 *   line - configuration file line number
 ...
 *   attribute lists.  The <id> is auto-generated by the caller; the exception
 *   type and the message and attribute samples are filled in by the parser.
 *
 * RETURN VALUE
 *   Returns a pointer to the initialized structure, or NULL on failure.
 */
...
/***
 * NAME
 *   flt_otel_conf_exception_free - conf_exception structure deallocation
 *
 * SYNOPSIS
 *   void flt_otel_conf_exception_free(struct flt_otel_conf_exception **ptr)
 *
 * ARGUMENTS
 *   ptr - a pointer to the address of a structure
 *
 * DESCRIPTION
 *   Deallocates memory used by the flt_otel_conf_exception structure and its
 *   contents, then removes it from the list of structures of that type.
 *
 * RETURN VALUE
 *   This function does not return a value.
